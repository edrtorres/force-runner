...
import { optionalLimit, requiredString } from "../validation.ts";
...
export class SendReactionUseCase {
  ...
  async execute(userId: string, input: Record<string, unknown>) {
    ...
    const reactionType = requiredString(input, "reaction_type");
...
export class CoachAiUseCase {
  ...
  async execute(userId: string, input: Record<string, unknown>) {
    ...
    const caloriesGoal = typeof input.calories_goal === "number" ? input.calories_goal : null;
    ...
    return { coach_message: await this.coach.createMessage({ user_id: userId, input_type: typeof input.input_type === "string" ? input.input_type : "text", question, answer, calories_goal: caloriesGoal, metadata: input.metadata ?? {} }) };
  }
}
